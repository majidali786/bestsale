<?php
	class bankPaymentModel extends MY_Model{
	...
	$insertMain = $this->db->query("INSERT INTO $mainTable(NO,VDATE,CACODE,CANAME,ACODE,ANAME,DESCR,DEBIT,BCODE,BNAME,STATUS,U_ID,STATUS2,INVNO,INVAMT,CHQNO,CHQDATE,B_ID,UNPOSTED,POSTED,APPROVED) 
	VALUES ('$no','$vdate','".$data['cacode']."','$caname','$acode','$aname','$descrip','$debit','$bcode','$bname','1','".$this->userData['U_ID']."','1','$invno','$amt','$chqno','$chqdate','".$this->userData['B_ID']."','$unposted','$posted','$approved')");
	
	if($insertMain){
	if($data['rights']['UNPOSTED']==0){	
	$gnrllgrCash = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,CHQNO,CHQDATE,BCODE,BNAME) 
	VALUES ('$no','$vdate','$voucher_Jo','".$data['cacode']."','$descrip',0,$debit,'".$this->userData['B_ID']."','$chqno','$chqdate','$bcode','$bname')");
	
	$gnrllgrAcc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,CHQNO,CHQDATE,BCODE,BNAME,PNO)
	VALUES ('$no','$vdate','$voucher_Jo','$acode','$descrip',$debit,0,'".$this->userData['B_ID']."','$chqno','$chqdate','$bcode','$bname','$invno')");
	}
	}
