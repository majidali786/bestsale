<?php
	class cashPaymentMemoModel extends MY_Model{
	...
	$row=1;
	while($row<$nrows){
	if(!empty($data['acode_'.$row.'']) && !empty($data['invoices_'.$row.''])){
	...
	$insertMain = $this->db->query("INSERT INTO $mainTable(NO,VDATE,CACODE,CANAME,ACODE,ANAME,DESCR,DEBIT,STATUS,U_ID,STATUS2,INVNO,INVAMT,B_ID,UNPOSTED,POSTED,APPROVED) VALUES ('$no','$vdate','".$data['cacode']."','$caname','$acode','$aname','$descrip','$debit','1','".$this->userData['U_ID']."','1','$invno','$amt','".$this->userData['B_ID']."','$unposted','$posted','$approved')");
	
	if($insertMain){
	if($data['rights']['UNPOSTED']==0){	
	$gnrllgrCash = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID) VALUES ('$no','$vdate','$voucher_Jo','".$data['cacode']."','$descrip',0,$debit,'".$this->userData['B_ID']."')");
	
	
	$gnrllgrAcc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,MNO) VALUES ('$no','$vdate','$voucher_Jo','$acode','$descrip',$debit,0,'".$this->userData['B_ID']."','$invno')");
	}
	}
