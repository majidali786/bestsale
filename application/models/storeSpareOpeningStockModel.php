<?php
	class storeSpareOpeningStockModel extends MY_Model{
	...
	$insertRow = $this->db->query("INSERT INTO $mainTable2(NO,VDATE,PCODE,PNAME,UNIT,COIL,WEIGHT,QTY,FEET,RATE,AMOUNT,SNO,U_ID,B_ID) VALUES ('$no','$vdate','$pcode','$pname','$unit','$coil','$weight','$qty','$feet','$rate','$amount','$sno','".$this->userData['U_ID']."','".$this->userData['B_ID']."')");
	
	if($insertRow){
	if($data['rights']['UNPOSTED']==0){	
	$descrip="Opening Stock Qty".$qty." ".$weight."Kg Ft".$feet;	
	$stockt = $this->db->query("INSERT INTO $stock(NO,VDATE,JO,PCODE,DESCR,INWGHT,OUTWGHT,INQT,OUTQT,INFT,OUTFT,INAMT,OUTAMT,RATE,B_ID,DPCODE) VALUES ('$no','$vdate','$voucher_Jo','$pcode','$descrip',$weight,0,$qty,0,$feet,0,$amount,0,$rate,'".$this->userData['B_ID']."','$dpcode')");
