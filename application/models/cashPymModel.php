<?php
	class cashPymModel extends MY_Model{		
   ...
   //$this->deleteData("$gnrllgrChq",array("Jo"=>"$voucher_Jo","NO"=>$no,"B_ID"=>$this->userData['B_ID']));		
	...
	$prebal=$prev[0]['TDB'];
	$totbal=$prebal-$debit;
	$message="Cash Payment Voucher".PHP_EOL." ".$data['vdate']." ".PHP_EOL." C.R ".$no." ".PHP_EOL." ".$aname."".PHP_EOL."  P.B=".$prebal." ".PHP_EOL." Payment: ".$debit." ".PHP_EOL." Remaining Balance=".$totbal." ".PHP_EOL." If you feel any problem regard this then must contact with in 7 days 03214426383;";	
	...
	$gnrllgrCash = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,SNO) VALUES ('$no','$vdate','$voucher_Jo','".$data['cacode']."','$descrip',0,'$debit','".$this->userData['B_ID']."','$row')");
	
	$gnrllgrAcc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,SNO) VALUES ('$no','$vdate','$voucher_Jo','$acode','$descrip','$debit',0,'".$this->userData['B_ID']."','$row')");
	}
	}		
	}
	}
