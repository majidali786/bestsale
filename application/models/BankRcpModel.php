<?php
	class BankRcpModel extends MY_Model{		
   ...
   //$this->deleteData("$gnrllgrChq",array("Jo"=>"$voucher_Jo","NO"=>$no,"B_ID"=>$this->userData['B_ID']));		
	...
	VALUES ('$no','$vdate','$voucher_Jo','".$data['cacode']."','$descrip','$debit',0,'$chqno','$chqdate','".$this->userData['B_ID']."','$row','$acode','$aname')");
	
	$gnrllgrAcc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,CHQNO,CHQDATE,B_ID,SNO,FCODE,FNAME)
	VALUES ('$no','$vdate','$voucher_Jo','$acode','$descrip',0,'$debit','$chqno','$chqdate','".$this->userData['B_ID']."','$row','".$data['cacode']."','$caname')");
