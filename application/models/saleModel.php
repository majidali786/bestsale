<?php
	...
	$cbm=$this->getData("SALESMAN",array("ID"=>$saleman),"NAME","");
	if(!empty($cbm)){
	$slname=$cbm[0]['NAME'];		
	}
...
/* 	$st=array("Cash","Credit");
	...
	$net = str_replace(',','',$net);
    $rect=$data['rect'];
	$rect = str_replace(',','',$rect);
	
	
	if(empty($error)){
	
	$insertMain = $this->db->query("INSERT INTO $mainTable(NO,VDATE,STYPE,STNAME,VCODE,VNAME,ADDR,SCODE,SNAME,MOBILE,EMAIL,SLCODE,SLNAME,RAMOUNT,REMARKS,VNO,TQTY,TOTAL,
	GST,GSTAMT,NET,TAX,WHOLES,U_ID,B_ID,UNPOSTED,POSTED,APPROVED) 
	VALUES ('$no','$vdate','0','','$vcode','$vname','$addr','$scode','$sname','$mobile','$email','$saleman','$slname','$rect','$remarks','$vno','$tqty','$tamount'
	...
	VALUES ('$no','$vdate','$voucher_Jo','30101001','$descrip',0,'$amount','".$this->userData['B_ID']."','".$this->userData['U_ID']."'
	,'$sno','$vcode','$vname','$vno')");
	//PARTY DEBIT
	...
	//SAELS ACCOUNT CREDIT
	$gnrllgrSacc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,U_ID,SNO,FCODE,FNAME,VNO) 
	VALUES ('$no','$vdate','$voucher_Jo','30101001','$descrip',0,'$net3','".$this->userData['B_ID']."','".$this->userData['U_ID']."'
	,'$sno','$vcode','$vname','$vno')");
	//PARTY DEBIT
	$gnrllgrPacc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,U_ID,SNO,FCODE,FNAME,VNO) 
	VALUES ('$no','$vdate','$voucher_Jo','$vcode','$descrip','$net3',0,'".$this->userData['B_ID']."','".$this->userData['U_ID']."'
	,'$sno','30101001','SALES A/C','$vno')");
	
	//SAELS TAX 
	$gnrllgrSacc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,U_ID,SNO,FCODE,FNAME,VNO) 
	VALUES ('$no','$vdate','$voucher_Jo','20102001','$descrip','0','$gst2','".$this->userData['B_ID']."','".$this->userData['U_ID']."'
	,'$sno','$vcode','$vname','$vno')");
	//PARTY DEBIT
	$gnrllgrPacc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,U_ID,SNO,FCODE,FNAME,VNO) 
	VALUES ('$no','$vdate','$voucher_Jo','$vcode','VAT TAX @5%','$gst2','0','".$this->userData['B_ID']."','".$this->userData['U_ID']."'
	,'$sno','30101001','SALES A/C','$vno')");
	...
	VALUES ('$no','$vdate','$voucher_Jo','$pcode','$descrip',0,'$qty','$vcode','".$this->userData['B_ID']."','$rate','$unit','$vno','".$this->userData['U_ID']."')");
	...
	VALUES ('$no','$vdate','$voucher_Jo','$scode','$vname','$rect',0,'".$this->userData['B_ID']."','".$this->userData['U_ID']."'
	,'$vcode','$vname','$vno')");
	
	//PARTY DEBIT
	$gnrllgrPacc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,U_ID,FCODE,FNAME,VNO) 
	VALUES ('$no','$vdate','$voucher_Jo','$vcode','$sname',0,'$rect','".$this->userData['B_ID']."'
	,'".$this->userData['U_ID']."','$scode','$sname','$vno')");
	}
