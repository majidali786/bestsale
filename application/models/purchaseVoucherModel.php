<?php
	class purchaseVoucherModel extends MY_Model{
	...
	VALUES ('$no','$vdate','$voucher_Jo','40101002','$vname','$gstamt',0,'".$this->userData['B_ID']."','".$this->userData['U_ID']."','$vcode','$vname')");
	
	//PARTY Credit
	$gnrllgrPacc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,U_ID,FCODE,FNAME) 
	VALUES ('$no','$vdate','$voucher_Jo','$vcode','PURCHASE DISCOUNT',0,'$gstamt','".$this->userData['B_ID']."','".$this->userData['U_ID']."','40101002','PURCHASE DISCOUNT')");
	...
	VALUES ('$no','$vdate','$voucher_Jo','40101001','$descrip','$damount',0,'".$this->userData['B_ID']."','".$this->userData['U_ID']."','$sno','$vcode','$vname')");
	//PARTY Credit
	$gnrllgrPacc = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,U_ID,SNO,FCODE,FNAME) 
	VALUES ('$no','$vdate','$voucher_Jo','$vcode','$descrip',0,'$damount','".$this->userData['B_ID']."','".$this->userData['U_ID']."','$sno','40101001','PURCHASE A/C')");
	
	
	$asd = $this->db->query("INSERT INTO $stock(NO,VDATE,JO,PCODE,DESCR,INQT,OUTQT,INAMT,OUTAMT,VCODE,B_ID,RATE,UNIT,VNO) 
	VALUES ('$no','$vdate','$voucher_Jo','$pcode','$descrip','$qty',0,'$damount',0,'$vcode','".$this->userData['B_ID']."','$rate','$unit','$vno')");
