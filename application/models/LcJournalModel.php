<?php
	class LcJournalModel extends MY_Model{
	...
	$insertMain = $this->db->query("INSERT INTO $mainTable(NO,VDATE,ACODE,ANAME,DESCR,LCNO,DEBIT,CREDIT,U_ID,B_ID,UNPOSTED,POSTED,APPROVED) VALUES ('$no','$vdate','$acode','$aname','$descrip','$lcno','$debit','$credit','".$this->userData['U_ID']."','".$this->userData['B_ID']."','$unposted','$posted','$approved')");
	
	if($insertMain){
	if($data['rights']['UNPOSTED']==0){	
	$gnrllgrCash = $this->db->query("INSERT INTO $gnrllgr(No,VDate,Jo,ACode,Descr,Debit,Credit,B_ID,LCNO) VALUES ('$no','$vdate','$voucher_Jo','$acode','$descrip',$debit,$credit,'".$this->userData['B_ID']."','$lcno')");
	}
	}
