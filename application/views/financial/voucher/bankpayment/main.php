...
<td><select name="invoices_e" id="invoices_e" value-splitter=","  change-assign-value="invAmt_e" style="text-align:center;" value-index="1" ajax-data class="form-control select2">
<option value="">Select Invoice</option>

</select></td>
<td><input type="text"  class="form-control th-right" name="invAmt_e" style="text-align:center;" readonly placeholder="Invoice Amount"></td>
</tr>
<tr class="theme-bg">
<th style="text-align:center;">Description</th>
<th style="text-align:center;">Cheque No. <button type="button" class="btn btn-info" reload-chqs ><i class="icon-reload"></i></button></th>
...
<td><input type="text" class="form-control" style="text-align:center;" name="chqNo_e" placeholder="Cheque No."></td>
<td><input type="text" class="form-control" style="text-align:center;" name="chqDate_e" input-mask-date placeholder="dd/mm/yyyy"></td>
...
<div class="modal fade modal-chq-book" id="basic" tabindex="-1" role="basic" aria-hidden="true">
<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header modal-header-custom">
<button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
<h4 class="modal-title">Available Cheques</h4>
</div>
<div class="modal-body"></div>
</div>
</div>
...
baseUrl="<?= base_url("financial/bank-payment")?>";	
});
...
if($(this).val()!=''){
// $(document).find(".modal-chq-book").find(".modal-dialog").css("width","60%");	
// $('.data-row').customPreloader("show");	
// var url="<?= base_url("data/chq-book");?>";
// $.get(url,data,function(response){
// $('.data-row').customPreloader("hide");
// $(document).find(".modal-chq-book").find(".modal-body").html(response.chqbook);			
// },'json');	
if($(document).find(".modal-pending-invoices").find(".modal-body").html()!=""){	
$(document).find(".modal-pending-invoices").modal();
}
//$(document).find(".modal-chq-book").modal();
setTimeout(function(){$(document).find("[name=invoices]").select2("close");},100);
}
});
// $(document).on("click","[reload-chqs]",function(){	
// var data={
	// bcode:$("[name=bank_e]").val()
// }	
// if($("[name=bank_e]").val()!=''){
// $(document).find(".modal-chq-book").find(".modal-dialog").css("width","60%");	
// $('.data-row').customPreloader("show");	
