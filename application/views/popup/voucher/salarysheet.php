...
<h2 class="margin-0"><b><i class="icon-basket"></i> Salary Sheet : <?= $branch[$data1[0]['B_ID']]?></b></h2>
</div>
...
<?php 
$totalDays=cal_days_in_month(CAL_GREGORIAN, date("n",strtotime($data1[0]['VDATE'])), date("Y",strtotime($data1[0]['VDATE'])));
?>
<div class="note note-warning margin-bottom-10 col-sm-12">
<h3 class="margin-0"><b><i class="icon-trophy"></i> Emplopyee Salary Sheet Of Month <i class="icon-calendar"></i> ( <?= date("F",strtotime($data1[0]['VDATE']))?>, Total Days:<?= $totalDays?>)</b></h3>
</div>
