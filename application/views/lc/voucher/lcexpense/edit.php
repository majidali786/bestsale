...
<div class="col-md-12 show-error">
<input type="text" class="form-control move-enter-up move-enter-3" data-position="3" input-mask-date name="tdate" value="<?= $tdate?>" placeholder="dd/mm/yyyy">
</div>
</div>
</div>
<div class="col-sm-2">
...
<div class="col-md-12 show-error">
<select name="caname" id="caname" change-assign-value="cacode" class="form-control select2 move-enter-up move-to-row move-enter-4" data-position="4">
...
<tr data-id="<?= $rowNumber;?>">
    ...
    <td><span><?= $row['LCCODE']."-".$row['LCNAME'];?></span>
        <input type="hidden" name="lcaccount_<?= $rowNumber;?>" value="<?= $row['LCCODE']."-".$row['LCNAME'];?>" hidden-data="lcid_<?= $rowNumber;?>" class="form-control">
        <input type="hidden" name="lcid_<?= $rowNumber;?>" value="<?= $row['LCCODE'];?>">
    </td>
	<td><span><?= $row['LCNO'];?></span>
        <input type="hidden" name="aname_<?= $rowNumber;?>" value="<?= $row['LCNO'];?>" hidden-data="acode_<?= $rowNumber;?>" class="form-control">
        <input type="hidden" name="acode_<?= $rowNumber;?>" value="<?= $row['LCNO'];?>">
    </td>
    ...
    <td class="th-right popovers" data-container="body" data-trigger="hover" data-placement="bottom" data-content="<?= numbertowords($row['DEBIT'])?>"><span><?= lakhseparater($row['DEBIT']);?></span>
        <input type="hidden" name="amount_<?= $rowNumber;?>" value="<?= lakhseparater($row['DEBIT']);?>" class="form-control sum_amount">
    </td>
</tr>
<?php 
$rowNumber ++;
...
$("[name=lclocation]").val(<?= $data[0]['LCLID']?>);
